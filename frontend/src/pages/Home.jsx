...
import { getUserRide, viewUserJoinedRides, viewUserCreatedRides } from "../api/apiUserRide";
...
const Home = () => {
  ...
  const [showAllUserRides, setShowAllUserRides] = useState(false);
  const [showAllAgencyRides, setShowAllAgencyRides] = useState(false);
  ...
  useEffect(() => {
    ...
    if (section === "user-rides") {
      ...
      setTimeout(() => {
        userRideRef.current?.scrollIntoView({ behavior: "smooth", block: "start", });
      }, 150);
      ...
      setTimeout(() => {
        agencyRideRef.current?.scrollIntoView({ behavior: "smooth", block: "start", });
      }, 150);
    }
  }, [location]);
        ...
        <ContentWrapper ref={userRideRef}>
          <HeroSection>
            <HeroContent>
              ...
              <Subtitle>
                Connect with riders travelling on the same route and enjoy affordable, eco-friendly journeys together.
              </Subtitle>
            </HeroContent>
            {filteredUserRides.length > 4 && (
              <ViewAllButton
                onClick={() => setShowAllUserRides(!showAllUserRides)}>
                {showAllUserRides ? "Show Less" : "View All"}
              </ViewAllButton>
            )}
          </HeroSection>
          <RideGrid
        ...
        <ContentWrapper ref={agencyRideRef}>
          <HeroSection>
            <HeroContent>
              ...
              <Subtitle>
                Book verified agency rides with comfortable seating and professional travel experience.
              </Subtitle>
            </HeroContent>
            {filteredAgencyRides.length > 4 && (
              <ViewAllButton
                onClick={() => setShowAllAgencyRides(!showAllAgencyRides)}>
                {showAllAgencyRides ? "Show Less" : "View All"}
              </ViewAllButton>
            )}
          </HeroSection>
          <RideGrid
