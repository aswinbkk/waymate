...
const MyTrip = () => {
  ...
  const fetchOfferedRides = async () => {
    ...
    try {
      ...
      setCreatedRideIds(response.createdRideIds?.map(String) || []);
  ...
  const fetchJoinedRides = async () => {
    try {
      ...
      setJoinedRideIds(
        response.joinedRideIds?.map(String) || []);
  ...
  // STATUS
  const rideId = String(selectedRide?._id || selectedRide?.id || "");
  ...
  const onLeaveRide = async () => {
      ...
      setJoinedRideIds( (prev) => prev.filter( (id) => id !== rideId ) );
      setJoinedRides( (prev) => prev.filter( (ride) => String( ride._id || ride.id ) !== rideId ) );
      closeRidePopup();
    };
